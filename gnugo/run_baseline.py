# GNU Go baseline
#
# GNU Go plays the black against one heuristic opponent (white), 
# with go_v5 refereeing + area-scoring. Logs to W&B with the same metric names as the agents
# and maps the games across a step axis so the baseline overlays the agents' training charts as a
# flat reference line. GNU Go does NOT learn, so a few hundred games is a complete baseline.
...
# go_v5's agent names for the two colours
BLACK, WHITE = "black_0", "white_0"

# read the stones each colour occupies straight from the go_v5 observation
def stones_from_obs(obs, agent, n):
    board = obs["observation"]
    own, opp = board[:, :, 1], board[:, :, 0]
    black_plane, white_plane = (own, opp) if agent == BLACK else (opp, own)

    # convert a stone plane into the set of go_v5 action indices it occupies
    def acts(plane):
...
# Play one game - GNU Go (black) vs bot (white). Returns (won - 0/1).
def play_game(gnugo, bot, n, komi, seed, max_moves, verify):
    ...
    while env.agents:
        ...
        # drain a finished agent's turn without acting
        if term or trunc:
            env.step(None)
        ...
        # GNU Go's move or the heuristic opponent's — apply it and check
        if agent == BLACK:
            a = gnugo.genmove()
            if a == "resign":
                return 0, moves
        ...
        # optional board-sync check - catches any coordinate mapping bug early
        ...
        # safety cap so a stuck game still ends and gets scored
        if moves >= max_moves:
            force_finish(env, n)
            break
    return (1 if black_reward > 0 else 0), moves

# entry point - parse args, play the games, write the results CSV
def main():
    ...
    # optional W&B logging, mapped onto the same step axis as the agents
    run = None
    ...
    # launch the engine, capturing its stderr for crash diagnosis
    ...
    try:
        ...
        # play games until the target count is reached
        while completed < args.games:
            ...
            try:
                ...
                # give up on this opponent after too many crashes in a row
                if consec_fail >= 10:
            ...
            # record the completed game and update the rolling metrics
            consec_fail = 0
            attempt += 1
            completed += 1
            wins += won
            ...
            if run is not None:
                step = int(completed / args.games * args.span)   
                run.log({
                    "custom/win_rate": win_rate,
                    "custom/total_episodes": completed,
                    "rollout/ep_rew_mean": 2.0 * win_rate - 1.0,   
    ...
    # final summary - overall win rate written to CSV and W&B
