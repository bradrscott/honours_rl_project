...
#SBATCH --job-name="P2FEU9"
#SBATCH --array=0-8
#SBATCH --output=slurm-%x-%a-%j.out
#SBATCH --mail-user=sctbra008@myuct.ac.za
#SBATCH --mail-type=ALL

# Phase 2: Feudal abrupt-shift & recovery — UCT CPU (ada), 9x9
# 9 conditions = 3 magnitudes x 3 frequencies (array 0-8)
# Each resumes the 9x9 Phase-1 feudal checkpoint of opponent A and applies a shift schedule, measuring recovery (W=100).
# sbatch run_phase2_feudal_9x9_uct.sh
...
export OPPONENT=$A
export RESUME_FROM=./phase1_checkpoints/feudal/${BOARD_SIZE}x${BOARD_SIZE}/${A}/feudal_go_final.pt
